...
export default function Hero({ onSelectCountry, onStartQuiz }: HeroProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setHighlightedIndex((p) => Math.min(p + 1, results.length - 1)); }
    else if (e.key === "ArrowUp") { e.preventDefault(); setHighlightedIndex((p) => Math.max(p - 1, -1)); }
    else if (e.key === "Enter" && highlightedIndex >= 0) { handleSelect(results[highlightedIndex]); }
    else if (e.key === "Escape") { setIsFocused(false); setResults([]); }
  };

  const handleSelect = (country: CountryData) => {
    setSearchQuery(""); setResults([]); setIsFocused(false);
  ...
  return (
    <section id="globe" className="relative min-h-[100dvh] flex flex-col justify-center px-5 md:px-12 pt-16" style={{ zIndex: 1, pointerEvents: 'none' }}>
      <div className="max-w-[1400px] mx-auto w-full">
        ...
        <div className="relative max-w-[640px] mx-auto mt-8 lg:mt-0 animate-fade-in-up" style={{ animationDelay: "0.4s", pointerEvents: 'auto' }}>
          <div className="flex items-center gap-3 px-5 py-4 transition-all duration-200"
            ...
            <input ref={inputRef} type="text" placeholder="Search for a country..." value={searchQuery}
              ...
              onBlur={() => setTimeout(() => setIsFocused(false), 200)} onKeyDown={handleKeyDown}
          ...
          {results.length > 0 && isFocused && (
            <div className="absolute top-full left-0 right-0 mt-2 animate-fade-in"
              style={{ backgroundColor: "#13131f", border: "1px solid #1e1e2d", borderRadius: "16px", maxHeight: "400px", overflowY: "auto", zIndex: 100 }}>
              {results.map((country, index) => (
                <button key={country.id}
                  className="w-full flex items-center gap-4 px-5 py-4 transition-colors duration-100 text-left"
                  style={{ backgroundColor: highlightedIndex === index ? "rgba(201, 162, 39, 0.08)" : "transparent", borderBottom: index < results.length - 1 ? "1px solid #1e1e2d" : "none", pointerEvents: 'auto' }}
